...
export function ShiftManagement({ session }: { session: Session }) {
  ...
  const printOwn = async () => { try { const data = await api.printShifts(session.accessToken, month, !manager); const rows = data.assignments.map((item) => `<tr><td>${item.staffName}</td><td>${item.date}（${item.weekday}）</td><td class="${printedShiftClass(item.shiftType)}">${item.shiftType}</td><td>${item.assignedClass}</td><td>${item.startTime ?? ''}${item.endTime ? `〜${item.endTime}` : ''}</td></tr>`).join(''); const popup = window.open('', '_blank', 'noopener,noreferrer'); if (!popup) throw new Error('印刷画面を開けませんでした。ブラウザのポップアップ設定をご確認ください。'); popup.document.write(`<!doctype html><html lang="ja"><head><title>AeN Shift 月間シフト</title><style>@page{size:A4 landscape}body{font-family:sans-serif}table{border-collapse:collapse;width:100%}th,td{border:1px solid #888;padding:5px}.shift-normal{background:#e7f3ed;color:#174c38}.shift-early{background:#ffedd5;color:#9a3412}.shift-late{background:#dbeafe;color:#172554}.shift-off{background:#fafbf9;color:#52615a}@media print{button{display:none}*{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><h1>AeN Shift｜${data.tenantName} ${data.month} 月間シフト</h1><p>${data.status === 'CONFIRMED' ? '確定済み' : '下書き'}</p><table><tr><th>職員</th><th>日付</th><th>勤務区分</th><th>配置</th><th>勤務時間</th></tr>${rows}</table><button onclick="window.print()">印刷／PDFとして保存</button></body></html>`); popup.document.close(); } catch (error) { setMessage(error instanceof Error ? error.message : '印刷画面を表示できませんでした。時間をおいてもう一度お試しください。'); } };
