...
import { adminPrintHtml, openPrintWindow, personalCalendarPrintHtml } from '../exports/shift-print-layout';
...
export function ShiftManagement({ session }: { session: Session }) {
  ...
  const printOwn = async () => { try { const data = await api.printShifts(session.accessToken, month, !manager); openPrintWindow(manager ? adminPrintHtml(data) : personalCalendarPrintHtml(data)); } catch (error) { setMessage(error instanceof Error ? error.message : '印刷画面を表示できませんでした。時間をおいてもう一度お試しください。'); } };
