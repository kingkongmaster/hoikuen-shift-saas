...
import { StaffAttributeManagement } from './StaffAttributeManagement';
import { StaffWorkContractManagement } from './StaffWorkContractManagement';
...
export function StaffManagement({ token, readOnly = false }: { token: string; readOnly?: boolean }) {
  ...
  const [workRuleStaff,setWorkRuleStaff]=useState<Staff|null>(null);
  const [attributeStaff,setAttributeStaff]=useState<Staff|null>(null);
  const [contractStaff,setContractStaff]=useState<Staff|null>(null);
  ...
  return <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-xl font-bold">職員マスター</h3><p className="mt-1 text-sm text-slate-600">職員の登録・編集・無効化を管理します。</p></div>{!readOnly&&<button onClick={() => setEditing(null)} className="btn-primary"><span className="action-symbol bg-white/15" aria-hidden="true">職</span>職員を登録</button>}</div>
    ...
    {loading ? <p className="py-10 text-center text-slate-500">読み込み中…</p> : staff.length === 0 ? <p className="py-10 text-center text-slate-500">表示できる職員がいません。</p> : <>
      <div className="mt-5 hidden overflow-x-auto md:block"><table className="w-full min-w-[1200px] text-left text-sm"><thead className="bg-slate-50 text-slate-600"><tr>{['職員番号','氏名','役割・役職','雇用形態','担当クラス','勤務区分','通常勤務時間','月間目標','土曜','状態','操作'].map((label) => <th key={label} className="px-3 py-3 font-semibold">{label}</th>)}</tr></thead><tbody className="divide-y">{staff.map((member) => <tr key={member.id} className={!member.isActive ? 'bg-slate-50 text-slate-500' : ''}><td className="px-3 py-4 font-mono text-xs">{member.employeeNumber}</td><td className="px-3 py-4 font-medium">{member.displayName}</td><td className="px-3 py-4">{member.jobTitle ?? '未設定'}</td><td className="px-3 py-4">{employmentLabels[member.employmentType]}</td><td className="px-3 py-4">{classLabels[member.assignedClass]}</td><td className="px-3 py-4">{workLabels(member)}</td><td className="px-3 py-4">{regularHoursLabel(member)}</td><td className="px-3 py-4">{targetLabel(member)}</td><td className="px-3 py-4">{member.canWorkSaturdays ? '可' : '不可'}</td><td className="px-3 py-4"><Status active={member.isActive} /></td><td className="px-3 py-4"><Actions member={member} readOnly={readOnly} onEdit={() => setEditing(member)} onRules={() => setWorkRuleStaff(member)} onContracts={() => setContractStaff(member)} onDeactivate={() => setDeactivating(member)} /></td></tr>)}</tbody></table></div>
      <div className="mt-5 grid gap-4 md:hidden">{staff.map((member) => <article key={member.id} className={`rounded-xl border p-4 ${member.isActive ? 'border-slate-200' : 'border-slate-200 bg-slate-50 text-slate-500'}`}><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-xs text-slate-500">{member.employeeNumber}</p><h4 className="mt-1 text-lg font-bold">{member.displayName}</h4><p className="mt-1 text-sm text-slate-600">{member.jobTitle ?? '役割未設定'}</p></div><Status active={member.isActive} /></div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><Item label="雇用形態" value={employmentLabels[member.employmentType]} /><Item label="担当クラス" value={classLabels[member.assignedClass]} /><Item label="勤務区分" value={workLabels(member)} /><Item label="通常勤務時間" value={regularHoursLabel(member)} /><Item label="月間目標" value={targetLabel(member)} /><Item label="土曜日勤務" value={member.canWorkSaturdays ? '可' : '不可'} /></dl><div className="mt-4"><Actions member={member} readOnly={readOnly} onEdit={() => setEditing(member)} onRules={() => setWorkRuleStaff(member)} onContracts={() => setContractStaff(member)} onDeactivate={() => setDeactivating(member)} /></div></article>)}</div>
    </>}
    {staff.length>0&&<div className="mt-5 rounded-xl border border-slate-200 p-3"><p className="text-sm font-semibold">役職・資格・担当区分</p><p className="mt-1 text-sm text-slate-600">職員を選ぶと、シフト作成時の配置条件に使う情報を確認・編集できます。</p><div className="mt-2 flex flex-wrap gap-2">{staff.map(member=><button key={member.id} className="rounded-lg border border-sky-200 px-3 py-2 text-sm font-semibold text-sky-800" onClick={()=>setAttributeStaff(member)}>{member.displayName}さんの項目を開く</button>)}</div></div>}
    {editing !== undefined && <StaffFormModal staff={editing ?? undefined} saving={saving} onClose={() => setEditing(undefined)} onSave={save} />}
    {workRuleStaff && <StaffWorkRuleManagement token={token} staff={workRuleStaff} readOnly={readOnly} onClose={() => setWorkRuleStaff(null)} />}
    {attributeStaff && <StaffAttributeManagement token={token} staff={attributeStaff} readOnly={readOnly} onClose={() => setAttributeStaff(null)} />}
    {contractStaff && <StaffWorkContractManagement token={token} staff={contractStaff} onClose={() => setContractStaff(null)} />}
...
function Actions({ member, readOnly, onEdit, onDeactivate, onRules, onContracts }: { member: Staff; readOnly: boolean; onEdit: () => void; onDeactivate: () => void; onRules?:()=>void;onContracts?:()=>void }) { return <div className="flex flex-wrap gap-2">{!readOnly&&<button disabled={!member.isActive} onClick={onEdit} className="min-h-10 rounded-lg border border-slate-300 px-3 py-2 font-semibold disabled:cursor-not-allowed disabled:opacity-40">編集</button>}{onRules&&<button onClick={onRules} className="min-h-10 rounded-lg border border-emerald-200 px-3 py-2 font-semibold text-emerald-800">勤務条件</button>}{onContracts&&<button onClick={onContracts} className="min-h-10 rounded-lg border border-sky-200 px-3 py-2 font-semibold text-sky-800">勤務契約</button>}{!readOnly&&member.isActive&&<button onClick={onDeactivate} className="min-h-10 rounded-lg border border-rose-200 px-3 py-2 font-semibold text-rose-700">無効化</button>}</div>; }
