...
export type PaidLeaveGrant={id:string;staffId:string;grantDate:string;validFrom:string;expiresAt:string;grantedHalfDays:number;source:'NORMAL_GRANT'|'INITIAL_BALANCE'|'ADJUSTMENT';note:string|null;voidedAt:string|null;allocations:Array<{allocatedHalfDays:number}>};
export type PaidLeaveUsage={id:string;staffId:string;usageDate:string;unit:'DAY'|'HALF_DAY';usedHalfDays:number;status:'PROPOSED'|'CONFIRMED'|'CANCELLED'|'CORRECTED';decisionNote:string|null;cancellationReason:string|null;correctionReason:string|null;confirmedAt:string|null;confirmedByUserId:string|null;allocations:Array<{grantId:string;allocatedHalfDays:number}>};
export type PaidLeaveBalance={staffId:string;asOf:string;unit:'HALF_DAY';grantedHalfDays:number;usedHalfDays:number;availableHalfDays:number;grants:Array<{grantId:string;grantDate:string;validFrom:string;expiresAt:string;source:PaidLeaveGrant['source'];grantedHalfDays:number;usedHalfDays:number;remainingHalfDays:number;available:boolean}>};
...
export const api = {
  ...
  paidLeaveGrants(token:string,staffId:string){return request<PaidLeaveGrant[]>(`/staff/${staffId}/paid-leave/grants`,{},token);},
  createPaidLeaveGrant(token:string,staffId:string,input:{grantDate:string;validFrom:string;expiresAt:string;grantedHalfDays:number;source:PaidLeaveGrant['source'];note?:string|null}){return request<PaidLeaveGrant>(`/staff/${staffId}/paid-leave/grants`,{method:'POST',body:JSON.stringify(input)},token);},
  paidLeaveUsages(token:string,staffId:string){return request<PaidLeaveUsage[]>(`/staff/${staffId}/paid-leave/usages`,{},token);},
  paidLeaveBalance(token:string,staffId:string){return request<PaidLeaveBalance>(`/staff/${staffId}/paid-leave/balance`,{},token);},
  confirmPaidLeaveUsage(token:string,staffId:string,input:{usageDate:string;unit:'DAY'|'HALF_DAY';allocations:Array<{grantId:string;allocatedHalfDays:number}>;decisionNote:string}){return request<PaidLeaveUsage>(`/staff/${staffId}/paid-leave/usages/confirm`,{method:'POST',body:JSON.stringify(input)},token);},
  cancelPaidLeaveUsage(token:string,staffId:string,usageId:string,reason:string){return request<PaidLeaveUsage>(`/staff/${staffId}/paid-leave/usages/${usageId}/cancel`,{method:'POST',body:JSON.stringify({reason})},token);},
