...
const version2Arrays = [...version2RequiredArrays, 'workPatterns', 'staffWorkRules', 'staffAttributeDefinitions', 'staffAttributeAssignments', 'shiftStaffingRequirements'] as const;
const arrays = [...version2Arrays, 'staffWorkContracts', 'paidLeaveGrants', 'paidLeaveUsages', 'paidLeaveAllocations'] as const;
...
export class BackupsService {
  ...
  private async collect(tenantId: string): Promise<BackupData> {
    const [tenant, memberships, staff, shiftRequests, monthlyShifts, shiftAssignments, shiftSetting, classRequirements, closedDates, notifications, shiftSwapRequests, auditLogs, tenantFeatures, workPatterns, staffWorkRules, staffAttributeDefinitions, staffAttributeAssignments, shiftStaffingRequirements, staffWorkContracts, paidLeaveGrants, paidLeaveUsages, paidLeaveAllocations] = await Promise.all([
      ...
      this.prisma.shiftStaffingRequirement.findMany({ where: { tenantId } }),
      this.prisma.staffWorkContract.findMany({ where: { tenantId } }),
      this.prisma.paidLeaveGrant.findMany({ where: { tenantId } }),
      this.prisma.paidLeaveUsage.findMany({ where: { tenantId } }),
      this.prisma.paidLeaveAllocation.findMany({ where: { tenantId } }),
    ]);
    return { tenant: this.clean(tenant), members: memberships.map((item) => this.clean(item)), staff: staff.map((item) => this.clean(item)), shiftRequests: shiftRequests.map((item) => this.clean(item)), monthlyShifts: monthlyShifts.map((item) => this.clean(item)), shiftAssignments: shiftAssignments.map((item) => this.clean(item)), shiftSetting: shiftSetting ? this.clean(shiftSetting) : null, classRequirements: classRequirements.map((item) => this.clean(item)), closedDates: closedDates.map((item) => this.clean(item)), notifications: notifications.map((item) => this.clean(item)), shiftSwapRequests: shiftSwapRequests.map((item) => this.clean(item)), auditLogs: auditLogs.map((item) => this.clean(item)), tenantFeatures: tenantFeatures.map((item) => this.clean(item)), workPatterns: workPatterns.map((item) => this.clean(item)), staffWorkRules: staffWorkRules.map((item) => this.clean(item)), staffAttributeDefinitions:staffAttributeDefinitions.map(item=>this.clean(item)), staffAttributeAssignments:staffAttributeAssignments.map(item=>this.clean(item)), shiftStaffingRequirements:shiftStaffingRequirements.map(item=>this.clean(item)), staffWorkContracts:staffWorkContracts.map(item=>this.clean(item)), paidLeaveGrants:paidLeaveGrants.map(item=>this.clean(item)), paidLeaveUsages:paidLeaveUsages.map(item=>this.clean(item)), paidLeaveAllocations:paidLeaveAllocations.map(item=>this.clean(item)) };
  }

  private wrap(tenantId: string, data: BackupData) { const counts = this.counts(data); const integrity = { algorithm: 'SHA-256', checksum: this.checksum(data) }; return { format: 'enshift-backup', version: 3, exportedAt: new Date().toISOString(), tenantId, tenantName: String(data.tenant.name), application: 'EnShift', counts, data, integrity }; }
  private validateShape(value: unknown): any {
    ...
    if (backup.format !== 'enshift-backup' || backup.application !== 'EnShift') throw new BadRequestException('対応していないバックアップ形式です。'); if (![1, 2, 3].includes(backup.version)) throw new BadRequestException('対応していないバックアップversionです。'); if (!backup.tenantId || !backup.tenantName || !backup.data || !backup.counts || !backup.integrity) throw new BadRequestException('バックアップの必須項目が不足しています。'); if (backup.integrity.algorithm !== 'SHA-256' || typeof backup.integrity.checksum !== 'string') throw new BadRequestException('チェックサム情報が正しくありません。');
    ...
    for (const key of ['staffAttributeDefinitions','staffAttributeAssignments','shiftStaffingRequirements']) if (backup.version===2&&Object.prototype.hasOwnProperty.call(backup.data,key)&&(!Array.isArray(backup.data[key])||backup.counts[key]!==backup.data[key].length)) throw new UnprocessableEntityException(`バックアップ件数が一致しません: ${key}`);
    ...
    this.validateStaffingRequirements(backup);
    this.validatePaidLeave(backup);
    return backup;
  }
  private counts(data: BackupData) { return Object.fromEntries(arrays.map((key) => [key, data[key].length])); }
  private arrayKeys(version: number): readonly (typeof arrays)[number][] { return version === 1 ? legacyArrays : version === 2 ? version2Arrays : arrays; }
  private requiredArrayKeys(version: number): readonly string[] { return version === 1 ? legacyArrays : version === 2 ? version2RequiredArrays : arrays; }
  ...
  private validateStaffingRequirements(backup:any){const rows=Array.isArray(backup.data.shiftStaffingRequirements)?backup.data.shiftStaffingRequirements:[];const definitions=Array.isArray(backup.data.staffAttributeDefinitions)?backup.data.staffAttributeDefinitions:[];const definitionIds=new Set(definitions.map((row:any)=>row.id));const codes=new Set<string>();const levels=new Set(['HARD','SOFT','INFO']);const classes=new Set(['AGE_0','AGE_1','AGE_2','AGE_3','AGE_4','AGE_5','FREE','SUPPORT']);const control=/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/;for(const row of rows){if(row.tenantId!==backup.tenantId||!definitionIds.has(row.attributeDefinitionId)||!/^[A-Z][A-Z0-9_]{0,49}$/.test(row.code)||codes.has(row.code)||typeof row.name!=='string'||!row.name.trim()||row.name.length>100||row.classType!=null&&!classes.has(row.classType)||row.dayOfWeek!=null&&(!Number.isInteger(row.dayOfWeek)||row.dayOfWeek<0||row.dayOfWeek>6)||(row.startDate==null)!==(row.endDate==null)||row.startDate!=null&&row.startDate>row.endDate||!Number.isInteger(row.requiredCount)||row.requiredCount<1||!levels.has(row.constraintLevel)||!Number.isInteger(row.displayOrder)||row.displayOrder<0||row.displayOrder>100000||typeof row.isActive!=='boolean')throw new UnprocessableEntityException('属性別配置条件が正しくありません。');if(typeof row.reason==='string'&&(row.reason.length>500||control.test(row.reason)))throw new UnprocessableEntityException('属性別配置条件の理由が正しくありません。');if(row.startDate!=null&&row.startDate===row.endDate&&row.dayOfWeek!=null&&new Date(`${row.startDate}T00:00:00.000Z`).getUTCDay()!==row.dayOfWeek)throw new UnprocessableEntityException('属性別配置条件の特定日と曜日が一致しません。');codes.add(row.code);}const active=rows.filter((row:any)=>row.isActive);for(let i=0;i<active.length;i++)for(let j=i+1;j<active.length;j++){const a=active[i],b=active[j];if(a.attributeDefinitionId===b.attributeDefinitionId&&a.classType===b.classType&&a.dayOfWeek===b.dayOfWeek&&this.attributePeriodsOverlap(a,b))throw new UnprocessableEntityException('属性別配置条件の期間が重複しています。');}}
  private validatePaidLeave(backup:any){if(backup.version<3)return;const staffIds=new Set(backup.data.staff.map((row:any)=>row.id));const grantIds=new Set(backup.data.paidLeaveGrants.map((row:any)=>row.id));const usageIds=new Set(backup.data.paidLeaveUsages.map((row:any)=>row.id));for(const row of backup.data.staffWorkContracts)if(row.tenantId!==backup.tenantId||!staffIds.has(row.staffId))throw new UnprocessableEntityException('別Tenantまたは存在しない職員を参照する勤務契約があります。');for(const row of backup.data.paidLeaveGrants)if(row.tenantId!==backup.tenantId||!staffIds.has(row.staffId)||!Number.isInteger(row.grantedHalfDays)||row.grantedHalfDays<1||row.validFrom>row.expiresAt)throw new UnprocessableEntityException('有給付与記録が正しくありません。');for(const row of backup.data.paidLeaveUsages)if(row.tenantId!==backup.tenantId||!staffIds.has(row.staffId)||!['DAY','HALF_DAY'].includes(row.unit)||!Number.isInteger(row.usedHalfDays)||row.usedHalfDays!==(row.unit==='DAY'?2:1)||row.status==='PROPOSED'&&row.confirmedAt!=null)throw new UnprocessableEntityException('有給取得記録が正しくありません。');for(const row of backup.data.paidLeaveAllocations)if(row.tenantId!==backup.tenantId||!usageIds.has(row.usageId)||!grantIds.has(row.grantId)||!Number.isInteger(row.allocatedHalfDays)||row.allocatedHalfDays<1)throw new UnprocessableEntityException('有給消化割当が正しくありません。');}
