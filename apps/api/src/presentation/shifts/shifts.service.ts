...
export class ShiftsService {
  ...
  async generate(user: AuthenticatedUser, id: string) {
    ...
    const [staff, requests, setting, requirements, closedDates, managerMemberships, systemPatterns, staffingRequirements, staffAttributeAssignments, generatorExclusions, staffWorkRules] = await Promise.all([
      ...
      workRuleFeatureEnabled ? this.prisma.staffWorkRule.findMany({ where: { tenantId: user.tenantId, isActive: true, staff: { isActive: true }, OR: [{ startDate: null, endDate: null }, { startDate: { lt: range.end }, endDate: { gte: range.start } }] }, select: { id: true, staffId: true, ruleType: true, dayOfWeek: true, startDate: true, endDate: true, startTime: true, endTime: true, numericValue: true, priority: true, isHardConstraint: true, workPattern: { select: { id: true, code: true, startTime: true, endTime: true, breakMinutes: true, isWorking: true, isActive: true } } } }) : Promise.resolve([]),
    ]);
    ...
    const [contracts, confirmedAssignments] = await Promise.all([
      this.prisma.staffWorkContract.findMany({ where: { tenantId: user.tenantId, staffId: { in: generationStaffIds }, effectiveFrom: { lt: fiscalRange.endExclusive }, OR: [{ effectiveTo: null }, { effectiveTo: { gte: fiscalRange.start } }] }, select: { staffId: true, effectiveFrom: true, effectiveTo: true, annualizedTargetMinutes: true, prescribedDailyMinutes: true, voidedAt: true } }),
      this.prisma.shiftAssignment.findMany({ where: { tenantId: user.tenantId, staffId: { in: generationStaffIds }, workDate: { gte: fiscalRange.start, lt: fiscalRange.endExclusive }, monthlyShift: { status: MonthlyShiftStatus.CONFIRMED } }, select: { staffId: true, workDate: true, shiftType: true, startTime: true, endTime: true, breakMinutes: true } }),
    ]);
    const annualFairnessByStaff = new Map(generationStaff.map((member) => { const workContracts = contracts.filter((contract) => contract.staffId === member.id); const assignments = confirmedAssignments.filter((assignment) => assignment.staffId === member.id); const progress = calculateAnnualFairnessProgress({ ...member, workContracts, assignments }, fiscalRange, effectiveSetting); if (progress.calculationStatus !== 'COMPLETE' || progress.target.annualTargetMinutes == null || progress.actual.fairnessActualMinutes == null) return [member.id, undefined] as const; const prescribedMinutesByDate: Record<string, number> = {}; for (let current = new Date(range.start); current < range.end; current.setUTCDate(current.getUTCDate() + 1)) { const resolved = resolveDailyPrescribedMinutes(current, workContracts, member, effectiveSetting).minutes; if (resolved != null) prescribedMinutesByDate[this.isoDate(current)] = resolved; } return [member.id, { annualTargetMinutes: progress.target.annualTargetMinutes, confirmedFairnessMinutes: progress.actual.fairnessActualMinutes, prescribedMinutesByDate }] as const; }));
    const staffingOptions = staffingRequirements.length ? { staffingRequirements, staffAttributeAssignments } : {};
    const workRuleOptions = staffWorkRules.length ? { staffWorkRules } : {};
    const generated = generateRuleBasedSchedule(schedule.targetMonth, generationStaff.map((item) => ({ ...item, isDirector: false, annualFairness: annualFairnessByStaff.get(item.id) })), requests, { ...effectiveSetting, directorClassPlacementMode: setting.directorClassPlacementMode as 'NONE' | 'SHORTAGE_ONLY' | 'NORMAL', classRequirements: requirements, closedDates, ...staffingOptions, ...workRuleOptions });
    ...
    await this.prisma.$transaction([
      ...
      this.prisma.shiftAssignment.createMany({ data: generated.assignments.map((assignment) => ({ tenantId: user.tenantId, monthlyShiftId: schedule.id, ...assignment, workPatternId: assignment.workPatternId ?? patternByCode.get(assignment.shiftType)?.id ?? null })) }),
    ]);
