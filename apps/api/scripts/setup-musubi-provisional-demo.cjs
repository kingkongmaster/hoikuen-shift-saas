...
const adminEmail = process.env.MUSUBI_DEMO_ADMIN_EMAIL; const adminPassword = process.env.MUSUBI_DEMO_ADMIN_PASSWORD;
if (!adminEmail || !adminPassword || adminPassword.length < 12) throw new Error('匿名デモ管理者のメールと12文字以上のパスワードを環境変数で指定してください。');
...
async function main() {
  const result = await prisma.$transaction(async (tx) => {
    ...
    const admin = await tx.user.upsert({ where: { email: adminEmail.toLowerCase() }, update: { displayName: '仮運用管理者', isActive: true }, create: { email: adminEmail.toLowerCase(), displayName: '仮運用管理者', passwordHash: passwordHash(adminPassword) } });
