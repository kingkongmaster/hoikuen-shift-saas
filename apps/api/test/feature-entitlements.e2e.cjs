...
async function main() {
  ...
  const backupResponse = await call('/backups/export', { method: 'POST' }, token); assert.equal(backupResponse.status, 201); assert.equal(backupResponse.body.version, 3); assert.ok(Array.isArray(backupResponse.body.data.tenantFeatures)); assert.ok(backupResponse.body.data.tenantFeatures.some((item) => item.featureCode === 'TENANT_CUSTOM_RULES'));
